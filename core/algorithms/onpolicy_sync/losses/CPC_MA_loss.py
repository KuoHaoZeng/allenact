...
class CPC_MA_loss(AbstractActorCriticLoss):
    ...
    def loss(  # type: ignore
        ...
        **kwargs
        ...
        prev_actions = prev_actions[1:].view(nt * ng)
        positive = positive.view(nt * ng)
        negative = negative.view(nt * ng)
        masks = batch["masks"][1:].view(nt * ng)
        internal_mask = internal_gt_negative[1:].view(nt * ng, -1)[torch.arange(nt * ng), prev_actions]

        pos_label = torch.ones(nt * ng).to(positive.device)
        neg_label = torch.zeros(nt * ng).to(negative.device)
        loss_pos_pos = torch.nn.functional.binary_cross_entropy_with_logits(positive, pos_label, reduce=False)
        loss_pos_neg = torch.nn.functional.binary_cross_entropy_with_logits(positive, neg_label, reduce=False)
        loss_pos = loss_pos_neg * internal_mask + loss_pos_pos * (1 - internal_mask)
