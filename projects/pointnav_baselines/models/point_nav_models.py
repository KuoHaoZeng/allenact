...
class PointNavMAActorCriticSimpleConvRNN(ActorCriticModel[CategoricalDistr]):
    def __init__(
        ...
        self.missing_action_embedding = nn.Embedding(
            num_embeddings=action_space.n + 1, embedding_dim=missing_action_embedding_dim
        )
    ...
    def get_missing_action_embedding(self, observations):
        return self.missing_action_embedding(observations[self.missing_action_uuid][:,:,0].to(torch.int64))
...
class PointNavMAInternalActorCriticSimpleConvRNN(ActorCriticModel[CategoricalDistr]):
    def __init__(
            self,
            action_space: gym.spaces.Discrete,
            observation_space: SpaceDict,
    ...
    def forward(  # type:ignore
        ...
        if not self.is_blind:
            ...
            perception_difference = perception_embed - last_perception_embed
            x = [perception_embed] + x
        ...
        #crippled_action_feature = self.intenral_crippled_action_embedding(internal_output)
...
class PointNavMAInternalCPCActorCriticSimpleConvRNN(ActorCriticModel[CategoricalDistr]):
    def __init__(
            self,
            action_space: gym.spaces.Discrete,
            observation_space: SpaceDict,
...
class PointNavMAKeyPointsVisualNPMActorCriticSimpleConvRNN(ActorCriticModel[CategoricalDistr]):
    def __init__(
            self,
            action_space: gym.spaces.Discrete,
            observation_space: SpaceDict,
        ...
        self.state_encoder = RNNStateEncoder(
            (0 if self.is_blind else self.recurrent_hidden_state_size)
            + self.coorinate_embedding_size + obstacle_state_hidden_dim * action_space.n + missing_action_embedding_dim,
            self._hidden_size,
            num_layers=num_rnn_layers,
            rnn_type=rnn_type,
            )
