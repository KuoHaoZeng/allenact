...
class PointNavDynamicsCorruptionTask(Task[IThorEnvironment]):
    _actions = (MOVE_AHEAD, MOVE_BACK, ROTATE_LEFT, ROTATE_RIGHT, LOOK_DOWN, LOOK_UP, END)
    ...
    def _step(self, action: Union[int, Sequence[int]]) -> RLStepResult:
        ...
        if action not in self.task_info["missing_action"]:
            ...
            if action_str == END:
                ...
                self.env.step({"action": action_str})
                self.last_action_success = self.env.last_action_success
            elif action_str in [MOVE_AHEAD, MOVE_BACK]:
                self.env.step({"action": action_str,
                               "moveMagnitude": 0.25})
                self.last_action_success = self.env.last_action_success
                pose = self.env.agent_state()
                self.path.append({k: pose[k] for k in ["x", "y", "z"]})
                self.task_info["followed_path"].append(pose)
            elif action_str in [ROTATE_RIGHT, ROTATE_LEFT]:
                self.env.step({"action": action_str,
                               "degrees": 30})
                self.last_action_success = self.env.last_action_success
                pose = self.env.agent_state()
            ...
            if action == 0 or action == 1:
                if self.task_info["move_rot_deg"] != 0:
                    self.env.step({"action": action_str,
                                   "moveMagnitude": self.task_info["move_mag"],
                                   "degrees": self.task_info["move_rot_deg"]})
                else:
                    self.env.step({"action": action_str,
                                   "moveMagnitude": self.task_info["move_mag"]})
            else:
                self.env.step({"action": action_str,
                               "degrees": self.task_info["rot_deg"]})
            self.last_action_success = self.env.last_action_success
            pose = self.env.agent_state()
            self.path.append({k: pose[k] for k in ["x", "y", "z"]})
            self.task_info["followed_path"].append(pose)
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return {
            **super(PointNavDynamicsCorruptionTask, self).metrics(),
            "success": self._success,  # False also if no path to target
            "total_reward": total_reward,
            "dist_to_target": dist2tget,
            "spl": spl,
